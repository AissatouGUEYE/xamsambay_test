...
<li class="breadcrumb-item">
  <a class="yellow-text">Modification Localité</a>
</li>
@endsection
  <div class="row">
    <form  id="formAddLangue" action="/localites/edit" method="POST">
      @csrf
    <div class="col s12">
      <div class="card">
        <div class="card-content pb-0">
          <div class="card-header mb-2">
            <h4 class="card-title">Modification Localité</h4>
          </div>
          <div class="card-body">
            
            <div class="row">

                <input id="id" name="id" value="{{ $localite[0]['id'] }}" hidden>

                <div class="input-field col s6">
                    <input id="localite" type="text" class="validate" name="localite" value="{{ $localite[0]['localite'] }}">                
                    <label class="active" for="commune">Nom de la localité</label>
                </div>
                <div class="input-field col s6">
                    <select class="select2 browser-default" id="id_commune" name="id_commune">
                        {{-- <option value="{{ $localite[0]['id_commune'] }}" selected>{{ $localite[0]['commune'] }}</option>
                        ...
                        @foreach ($communes as $item)
                          ...
                          @if($item['id'] == $localite[0]['id_commune'])
                               <option value="{{ $item['id'] }}" selected>{{ $item['commune'] }}</option>
                          @else
                               <option value="{{ $item['id'] }}">{{ $item['commune'] }}</option>
                          @endif
                    ...
                    <label class="active" for="id_commune">Nom de la commune</label>
                </div>
            </div>
